...
我们之前讲过函数调用的底层实现，我们说到根据SystemV调用约定，函数在被调用时，需要依靠rdi、rsi、rdx、r10、r9、r8来实现实参的传递。那么系统调用也是这样吗？——是的，系统调用也是的。我们还说过常用寄存器rax来传递返回值，那么系统调用也是吗？——是的，系统调用也是的。而且，在系统调用中，rax寄存器不仅仅用在系统调用函数返回时，还用在系统调用函数被调用前——用来传递函数号，告知操作系统被调用的是哪个函数。
